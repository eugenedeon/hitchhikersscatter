...
// GEN 3D HALF-SPACE - homogeneous half-space - vacuum index-matched boundary - generalized radiative transfer form
...
public: 
                                  ...
                                  )
    {
        ...
        for( size_t i = 0; i < numsamples; ++i )
        {
            ...
            {    
                ...
                expected_track_length += ( step <= pos.z ? step : (pos.z + step + pos.z * log( step/pos.z ) ) * 0.5 );
                expected_track_length2 += ( step <= pos.z ? step : (pos.z + step + pos.z * log( step/pos.z ) ) * 0.5 - 0.5 * step + 0.5 * cmfp() );
    ...
    // 2D (r-z geometry)
    // estimate albedo, internal collision density and radial exitance
    void HalfSpace2DEstimatorAnalog(    const double c,               // single-scattering albedo
                                        const double dz,              // resolution of depth-based quantities
                                        const double maxz,            // maximum depth recorded
                                        const double dr,
                                        const double maxr,
    ...
    {
        const size_t num_r_bins = size_t( floor( maxr / dr ) ) + 1;
        const size_t num_z_bins = size_t( floor( maxz / dz ) ) + 1;
        const size_t num_zr_bins = num_r_bins * num_z_bins;

        size_t countExitingParticles = 0; // used to estimate total albedo
        size_t countExitingParticlesSingle = 0; // used to estimate total albedo
        size_t countExitingParticlesDouble = 0; // used to estimate total albedo

        // internal density
        size_t * density = new size_t [num_zr_bins];
        for( size_t i = 0; i <= num_zr_bins; ++i )
        {
            density[i] = 0;
        }

        // radial exitance
        size_t * exitance = new size_t [num_r_bins];
        size_t * exitanceSingle = new size_t [num_r_bins];
        size_t * exitanceDouble = new size_t [num_r_bins];
        for( size_t i = 0; i < num_r_bins; ++i )
        {
            exitance[i] = 0.0;
            exitanceSingle[i] = 0.0;
            exitanceDouble[i] = 0.0;
        }
        ...
        for( size_t i = 0; i < numsamples; ++i )
        {
            ...
            {   
                if( pos.z <= 0.0 )
                {
                    //escape
                    countExitingParticles++;
                    if( 1 == collisionOrder ) countExitingParticlesSingle++;
                    if( 2 == collisionOrder ) countExitingParticlesDouble++;

                    // backup to the surface
                    pos += dir * (-pos.z / dir.z );
                    ...
                    exitance[ri]++;
                    if( 1 == collisionOrder ) exitanceSingle[ri]++;
                    if( 2 == collisionOrder ) exitanceDouble[ri]++;

                    break;
                }

                // collision - update histogram:
                collisionOrder++;
               
                const size_t zi = discreteMap( 0.0, maxz, dz, pos.z );
                const double r = sqrt( pos.x * pos.x + pos.y * pos.y );
                const size_t ri = discreteMap( 0.0, maxr, dr, r );

                density[zi * num_r_bins + ri]++;
        ...
        printDescriptor();
        std::cout << "Analog2Destimator c: " << c << 
                     " maxz: " << maxz << 
                     " dz: " << dz << 
                     " maxr: " << maxr << 
                     " dr: " << dr << 
        ...
        std::cout << "total exitance: " << double( countExitingParticles ) / double( numsamples ) << std::endl;
        std::cout << "single exitance: " << double( countExitingParticlesSingle ) / double( numsamples ) << std::endl;
        std::cout << "double exitance: " << double( countExitingParticlesDouble ) / double( numsamples ) << std::endl;

        std::cout << "Exitant radial distribution: " << std::endl;
        for( size_t i = 0; i < num_r_bins; ++i )
        {
            std::cout << double( exitance[i] ) / ( double( numsamples ) * dr ) << " ";
        }
        std::cout << std::endl;

        std::cout << "Exitant radial distribution single: " << std::endl;
        for( size_t i = 0; i < num_r_bins; ++i )
        {
            std::cout << double( exitanceSingle[i] ) / ( double( numsamples ) * dr ) << " ";
        }
        std::cout << std::endl;

        std::cout << "Exitant radial distribution double: " << std::endl;
        for( size_t i = 0; i < num_r_bins; ++i )
        {
            std::cout << double( exitanceDouble[i] ) / ( double( numsamples ) * dr ) << " ";
        }
        std::cout << std::endl;

        std::cout << "subsurface collision density: " << std::endl;
        for( size_t zi = 0; zi < num_z_bins; zi++ )
        {
            for( size_t ri = 0; ri < num_r_bins; ri++ )
            {
                std::cout << double( density[ zi * num_r_bins + ri ] / double( numsamples * dr * dz ) ) << " ";
            }
            std::cout << std::endl;
        }

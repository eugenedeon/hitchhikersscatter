...
// GEN - HALF ROD - base estimators for a variety of step-length distributions and phase functions for multiple scattering in
//                    a semi-infinite (half) rod - positive x is right - source is at x = 0
...
public: 
    ...
    void halfRodEstimatorAnalog(  const double c,           // single-scattering albedo
                                  ...
                                  )
    {
        ...
        const size_t num_x_bins = floor( maxx / dx ) + 1.0;
        ...
        for( size_t i = 0; i < numsamples; ++i )
        {
            ...
            while( true )
            {
                ...
                const size_t x_index = discreteMap( 0, maxx, dx, pos );
        ...
        std::cout << "total albedo: " << double( countExitingParticles ) / double( numsamples ) << std::endl;
        std::cout << "per-collision order albedos: " << std::endl;
        for( size_t i = 0; i < numCollisionOrders; ++i )
        {
        ...
        delete [] perOrderExitingParticles;
